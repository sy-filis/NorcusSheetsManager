...
internal class Transaction(string baseFolder, string invalidFullName, IEnumerable<IRenamingSuggestion> suggestions) : IRenamingTransaction
{
  ...
  public ITransactionResponse Commit(IRenamingSuggestion suggestion)
  {
    ...
    try
    {
      ...
      _logger.Debug("File name {InvalidPath} was corrected to {NewName}.", suggestion.InvalidFullPath, Path.GetFileName(suggestion.FullPath));
    }
    catch (Exception e)
    {
      _logger.Warn(e, "File {InvalidPath} could not be renamed to {NewPath}.", suggestion.InvalidFullPath, suggestion.FullPath);
